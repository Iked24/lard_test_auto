...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO parks (name, address, schedule) VALUES (?, ?, ?)");
            $stmt->execute([$parkData['name'], $parkData['address'], $parkData['schedule'] ?? null]);
            $parkId = $pdo->lastInsertId('parks_id_seq');
...
<html>
<head>
    <title>Создание автопарка</title>
